...
// Some helpful macros
#define DRAW_SPLASH()                                                          \
  nes_pixelcpy((uint32_t *)SPLASH_BASEADDR, WIDTH, HEIGHT, 0,                  \
               (uint16_t *)bootstate.activeBuffer, 0, NULL);
#define DRAW_BMP(i)                                                            \
  nes_pixelcpy(nesinfo[i].bmpAddr, BMP_WIDTH, BMP_HEIGHT, 0,                   \
               (uint16_t *)bootstate.activeBuffer + MASK_YOFFSET * WIDTH +     \
                   MASK_XOFFSET,                                               \
