...
void Map133_Apu(word wAddr, byte byData) {
  if (wAddr == 0x4120) {
    ...
    /* Set PPU Banks */
    W.PPUBANK[0] =
        VROMPAGE((((byData & 0x03) << 3) + 0) % (S.NesHeader.VROMSize << 3));
    W.PPUBANK[1] =
        VROMPAGE((((byData & 0x03) << 3) + 1) % (S.NesHeader.VROMSize << 3));
    W.PPUBANK[2] =
        VROMPAGE((((byData & 0x03) << 3) + 2) % (S.NesHeader.VROMSize << 3));
    W.PPUBANK[3] =
        VROMPAGE((((byData & 0x03) << 3) + 3) % (S.NesHeader.VROMSize << 3));
    W.PPUBANK[4] =
        VROMPAGE((((byData & 0x03) << 3) + 4) % (S.NesHeader.VROMSize << 3));
    W.PPUBANK[5] =
        VROMPAGE((((byData & 0x03) << 3) + 5) % (S.NesHeader.VROMSize << 3));
    W.PPUBANK[6] =
        VROMPAGE((((byData & 0x03) << 3) + 6) % (S.NesHeader.VROMSize << 3));
    W.PPUBANK[7] =
        VROMPAGE((((byData & 0x03) << 3) + 7) % (S.NesHeader.VROMSize << 3));
    NESCore_Develop_Character_Data();
  }
