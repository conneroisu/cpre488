...
void Map227_Write(word wAddr, byte byData) {
  ...
  if (!(wAddr & 0x0080)) {
    if (wAddr & 0x0200) {
      W.ROMBANK2 =
          ROMPAGE((((byBank & 0x1C) << 2) + 14) % (S.NesHeader.ROMSize << 1));
      W.ROMBANK3 =
          ROMPAGE((((byBank & 0x1C) << 2) + 15) % (S.NesHeader.ROMSize << 1));
    } else {
      W.ROMBANK2 =
          ROMPAGE((((byBank & 0x1C) << 2) + 0) % (S.NesHeader.ROMSize << 1));
      W.ROMBANK3 =
          ROMPAGE((((byBank & 0x1C) << 2) + 1) % (S.NesHeader.ROMSize << 1));
    }
  }
  if (wAddr & 0x0002) {
    NESCore_Mirroring(0);
