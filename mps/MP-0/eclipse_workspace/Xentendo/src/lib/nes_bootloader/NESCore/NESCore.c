...
void NESCore_Callback_Wait() {
  ...
  usec_this_frame =
      (1000000 * (tv.tv_sec - ptv.tv_sec)) + tv.tv_usec - ptv.tv_usec;
  if (ptv.tv_sec == 0) {
...
void NESCore_Init_PPU() {
  ...
  S.PPU_R0 = S.PPU_R1 = S.PPU_R2 = S.PPU_R3 = S.PPU_R7 = 0;
  S.PPU_Latch_Flag = 0;
  S.PPU_UpDown_Clip = 0;
  S.FrameStep = 0;
  S.FrameIRQ_Enable = 0;
...
void NESCore_Halt() { S.EmulatorRun = 0; }
  ...
  for (i = 0; i < 4; i++) {
    if (GG[i].code[0] != 0) {
      ...
      if (badcode)
        ...
        valid++;
        GG[i].wAddr = 0x8000 + (((n[3] & 7) << 12) | ((n[5] & 7) << 8) |
                                ((n[4] & 8) << 8) | ((n[2] & 7) << 4) |
                                ((n[1] & 8) << 4) | (n[4] & 7) | (n[3] & 8));
...
int NESCore_HSync() {
  ...
  S.PPU_Scanline =
      (S.PPU_Scanline == S.VBlankEndScanline) ? 0 : S.PPU_Scanline + 1;
...
void NESCore_DrawScanline() {
  ...
  if (!(S.PPU_R1 & R1_SHOW_SCR)) {
    ...
    if (S.FrameCnt == 0) {
      ...
      pPalTbl =
          &S.PalTable[(((pAttrBase[nX >> 2] >> ((nX & 2) + nY4)) & 3) << 2)];
    ...
    if (S.FrameCnt == 0) {
      ...
      if (S.PPU_UpDown_Clip &&
          (SCAN_ON_SCREEN_START > S.PPU_Scanline ||
           S.PPU_Scanline > SCAN_BOTTOM_OFF_SCREEN_START)) {
        word *pPointTop;
  ...
  if (S.FrameCnt == 0 && (S.PPU_R1 & R1_SHOW_SP)) {
    ...
    for (pSPRRAM = S.SPRRAM + (252); pSPRRAM >= S.SPRRAM; pSPRRAM -= 4) {
      ...
      nYBit = (nAttr & SPR_ATTR_V_FLIP) ? (S.PPU_SP_Height - nYBit - 1) << 3
                                        : nYBit << 3;
      ...
      if (S.PPU_R0 & R0_SP_SIZE) {
        ...
        if (pSPRRAM[SPR_CHR] & 1) {
          pbyChrData =
              S.ChrBuf + 0x4000 + ((pSPRRAM[SPR_CHR] & 0xfe) << 6) + nYBit;
        } else {
          pbyChrData = S.ChrBuf + ((pSPRRAM[SPR_CHR] & 0xfe) << 6) + nYBit;
        }
      } else {
        /* Sprite size 8x8 */
    ...
    for (nX = 0; nX < NES_DISP_WIDTH; ++nX) {
      ...
      if (nSprData &&
          (!(nSprData & 0x80) || ((S.NES_BG_Index_Temp[nX] & 3) == 0))) {
...
void NESCore_GetSprHitY() {
  ...
  if (S.PPU_R0 & R0_SP_SIZE) {
    ...
    if (S.SPRRAM[SPR_CHR] & 0x01) {
      ...
      pdwChrData =
          (dword *)(S.ChrBuf + ((S.SPRRAM[SPR_CHR] & 0xfe) << 6) + nYBit);
    }
  } else {
    /* Sprite size 8x8 */
