...
void Map242_Write(word wAddr, byte byData) {
  if (wAddr & 0x01) {
    /* Set ROM Banks */
    W.ROMBANK0 =
        ROMPAGE((((wAddr & 0xF8) >> 1) + 0) % (S.NesHeader.ROMSize << 1));
    W.ROMBANK1 =
        ROMPAGE((((wAddr & 0xF8) >> 1) + 1) % (S.NesHeader.ROMSize << 1));
    W.ROMBANK2 =
        ROMPAGE((((wAddr & 0xF8) >> 1) + 2) % (S.NesHeader.ROMSize << 1));
    W.ROMBANK3 =
        ROMPAGE((((wAddr & 0xF8) >> 1) + 3) % (S.NesHeader.ROMSize << 1));
  }
}
