...
void Map193_Sram(word wAddr, byte byData) {
  ...
  case 0x6000:
    W.PPUBANK[0] =
        VROMPAGE(((byData & 0xfc) + 0) % (S.NesHeader.VROMSize << 3));
    W.PPUBANK[1] =
        VROMPAGE(((byData & 0xfc) + 1) % (S.NesHeader.VROMSize << 3));
    W.PPUBANK[2] =
        VROMPAGE(((byData & 0xfc) + 2) % (S.NesHeader.VROMSize << 3));
    W.PPUBANK[3] =
        VROMPAGE(((byData & 0xfc) + 3) % (S.NesHeader.VROMSize << 3));
    NESCore_Develop_Character_Data();
    break;
  case 0x6001:
