...
void Map57_Write(word wAddr, byte byData) {
  ...
  case 0x8800:
    ...
    if (byData & 0x80) {
      W.ROMBANK0 = ROMPAGE((((byData & 0x40) >> 6) * 4 + 8 + 0) %
                           (S.NesHeader.ROMSize << 1));
      W.ROMBANK1 = ROMPAGE((((byData & 0x40) >> 6) * 4 + 8 + 1) %
                           (S.NesHeader.ROMSize << 1));
      W.ROMBANK2 = ROMPAGE((((byData & 0x40) >> 6) * 4 + 8 + 2) %
                           (S.NesHeader.ROMSize << 1));
      W.ROMBANK3 = ROMPAGE((((byData & 0x40) >> 6) * 4 + 8 + 3) %
                           (S.NesHeader.ROMSize << 1));
    } else {
      W.ROMBANK0 = ROMPAGE((((byData & 0x60) >> 5) * 2 + 0) %
                           (S.NesHeader.ROMSize << 1));
      W.ROMBANK1 = ROMPAGE((((byData & 0x60) >> 5) * 2 + 1) %
                           (S.NesHeader.ROMSize << 1));
      W.ROMBANK2 = ROMPAGE((((byData & 0x60) >> 5) * 2 + 0) %
                           (S.NesHeader.ROMSize << 1));
      W.ROMBANK3 = ROMPAGE((((byData & 0x60) >> 5) * 2 + 1) %
                           (S.NesHeader.ROMSize << 1));
    }
