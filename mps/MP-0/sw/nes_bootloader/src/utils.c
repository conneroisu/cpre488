  ...
  for (i = 0; i < srcHeight; i++) {
    ...
    for (j = 0; j < srcWidth; j++) {
      ...
      if (alpha <= 0x000F) {
        ...
        dR = (RED16(TEXT_COLOR) * (0xF - alpha) + RED16(*destPtr16) * alpha) /
             0xF;
        if (dR > 0xF)
          dR = 0xF;

        // Blend the green channel
        dG = (GREEN16(TEXT_COLOR) * (0xF - alpha) +
              GREEN16(*destPtr16) * alpha) /
             0xF;
        if (dG > 0xF)
          dG = 0xF;

        // Blend the red channel
        dB = (BLUE16(TEXT_COLOR) * (0xF - alpha) + BLUE16(*destPtr16) * alpha) /
             0xF;
        if (dB > 0xF)
          dB = 0xF;
