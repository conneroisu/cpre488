...
void NESCore_Callback_OutputFrame(word *WorkFrame) {
    ...
    uint32_t i, j, x, y, z, n;
    uint16_t *ptr = (uint16_t *)FBUFFER_BASEADDR;
    uint16_t tpixel;
    char str[50];

    //Resolution
    const int GivenResolutionX = 256;
	const int GivenResolutionY = 240;
	const int OutputResolutionX = 640;
	const int OutputResolutionY = 480;

	// Scale factors
    const int scaleX = OutputResolutionX/GivenResolutionX;
    const int scaleY = OutputResolutionY/GivenResolutionY;

    const int Borders = (((double)OutputResolutionX/GivenResolutionX * GivenResolutionX) - (scaleX * GivenResolutionX))/2;

    for(n = 0; n < OutputResolutionY; n ++){
        for(z = 0;z<Borders; z++){
        	ptr[n * OutputResolutionX + z] = 0x0000;
        	ptr[n * OutputResolutionX + OutputResolutionX-1-z] = 0x0000;
        }
    }

    for (i = 0; i < NES_DISP_HEIGHT; i++) {
        for (j = 0; j < NES_DISP_WIDTH; j++) {
            // Get the NES pixel color
            tpixel = NesPalette3[WorkFrame[NES_DISP_WIDTH * i + j]];

            int base_x = j * scaleX;
            int base_y = i * scaleY;

            // Fill the corresponding area in the framebuffer
            for (y = 0; y < scaleY; y++) {
                for (x = 0; x < scaleX; x++) {
                    ptr[(base_y + y) * OutputResolutionX + (base_x + x) + Borders] = tpixel;
                }
            }
        }
    }

    // Flush the cache to prevent VDMA issues
    Xil_DCacheFlush();
...
void NESCore_Callback_InputPadState(dword *pdwPad1, dword *pdwPad2) {
    ...
    *pdwPad1 = 0;
	...
	// BTNL
	if ((Xil_In32(ButtonIn) & 0x00000004) == 0x00000004) {
		*pdwPad1 = NCTL_LEFT;
		LEDOut = LEDOut + 0x00000010;
	}

	// BTNR
	if ((Xil_In32(ButtonIn) & 0x00000008) == 0x00000008) {
		*pdwPad1 = NCTL_RIGHT;
