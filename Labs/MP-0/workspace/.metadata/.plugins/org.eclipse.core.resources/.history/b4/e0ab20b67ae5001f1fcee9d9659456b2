...
void NESCore_Callback_OutputFrame(word *WorkFrame) {
    ...
    // Scale factors
    const int scaleX = 5 / 2;  // Approximate 2.5x scaling
    const int scaleY = 2;      // Exact 2x scaling
    ...
    for (i = 0; i < NES_DISP_HEIGHT; i++) {
        for (j = 0; j < NES_DISP_WIDTH; j++) {
            ...
            for (y = 0; y < scaleY; y++) {
                for (x = 0; x < scaleX; x++) {
                    ptr[(base_y + y) * 640 + (base_x + x)] = tpixel;
                }
            }
        }
    }
