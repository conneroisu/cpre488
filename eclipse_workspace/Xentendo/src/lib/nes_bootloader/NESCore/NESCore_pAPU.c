...
#include "../../../src/lib/nes_bootloader/NESCore/M6502_rw.h"
...
word ApuFreqLimit[8] = {0x3FF, 0x555, 0x666, 0x71C, 0x787, 0x7C1, 0x7E0, 0x7F0};

/* Noise Frequency Lookup Table */
dword ApuNoiseFreq[16] = {0x002, 0x004, 0x008, 0x010, 0x020, 0x030,
                          0x040, 0x050, 0x065, 0x07F, 0x0BE, 0x0FE,
                          0x17D, 0x1FC, 0x3F9, 0x7F2};

/* DMC Transfer Clocks Table */
dword ApuDpcmCycles[16] = {428, 380, 340, 320, 286, 254, 226, 214,
  ...
  while (event < S.cur_event) {
    if ((S.pAPU.ApuEventQueue[event].type & APUET_MASK) == APUET_C1) {
      ...
      case 2:
        ...
        S.pAPU.ApuC1Freq =
            ((((word)S.pAPU.ApuC1d & 0x07) << 8) + S.pAPU.ApuC1c);
...
void ApuRenderingWave1(void) {
  ...
  for (i = 0; i < ApuSamplesPerSync; i++) {
    ...
    if ((S.pAPU.ApuC1Freq < 8 ||
         (!MC1SweepIncDec && S.pAPU.ApuC1Freq > MC1FreqLimit))) {
      wave_buffers[0][i] = 0;
    ...
    if ((S.ApuCtrlNew & 0x01) && (S.pAPU.ApuC1Atl || MC1Hold)) {
      ...
      wave_buffers[0][i] =
          (S.pAPU.ApuC1Wave[S.pAPU.ApuC1Index >> 24] * S.pAPU.ApuC1Volume) >> 1;
    } else {
      wave_buffers[0][i] = 0;
    }
  }
  ...
  while ((event < S.cur_event)) {
    if ((S.pAPU.ApuEventQueue[event].type & APUET_MASK) == APUET_C2) {
      ...
      case 2:
        ...
        S.pAPU.ApuC2Freq =
            ((((word)S.pAPU.ApuC2d & 0x07) << 8) + S.pAPU.ApuC2c);
...
void ApuRenderingWave2(void) {
  ...
  for (i = 0; i < ApuSamplesPerSync; i++) {
    ...
    if (S.pAPU.ApuC2Freq < 8 ||
        (!MC2SweepIncDec && S.pAPU.ApuC2Freq > MC2FreqLimit)) {
      wave_buffers[1][i] = 0;
    ...
    if ((S.ApuCtrlNew & 0x02) && (S.pAPU.ApuC2Atl || MC2Hold)) {
      ...
      /* Fixed volume */
      wave_buffers[1][i] =
          (S.pAPU.ApuC2Wave[S.pAPU.ApuC2Index >> 24] * S.pAPU.ApuC2Volume) >> 1;
    } else {
      wave_buffers[1][i] = 0;
    }
  }
...
void ApuRenderingWave3(void) {
  ...
  for (i = 0; i < ApuSamplesPerSync; i++) {
    ...
    if ((S.ApuCtrlNew & 0x04) && (S.pAPU.ApuC3Atl > 0 || MC3Holdnote) &&
        S.pAPU.ApuC3Llc > 0) {
      S.pAPU.ApuC3Index += S.pAPU.ApuC3Skip;
      S.pAPU.ApuC3Index &= 0x1fffffff;
      wave_buffers[2][i] =
          triangle_50[S.pAPU.ApuC3Index >> 24] >> S.pAPU.ApuVolumeDivider;
      if (!S.BassBoost)
        wave_buffers[2][i] >>= 1;
    } else {
      wave_buffers[2][i] = 0;
  ...
  for (i = 0; i < ApuSamplesPerSync; i++) {
    ...
    if (S.ApuCtrlNew & 0x08) {
      ...
      if (S.pAPU.ApuC4Index > 0x1fffffff) {
        if (MC4Small)
          S.pAPU.ApuC4Sr |= ((!(S.pAPU.ApuC4Sr & 1)) ^ (!(S.pAPU.ApuC4Sr & 4)))
                            << 5;
        else
          S.pAPU.ApuC4Sr |= ((!(S.pAPU.ApuC4Sr & 1)) ^ (!(S.pAPU.ApuC4Sr & 16)))
                            << 9;
...
int ApuWriteWave5(int event) {
  while (event < S.cur_event) {
    if ((S.pAPU.ApuEventQueue[event].type & APUET_MASK) == APUET_C5) {
      S.pAPU.ApuC5Reg[S.pAPU.ApuEventQueue[event].type & 3] =
          S.pAPU.ApuEventQueue[event].data;

      switch (S.pAPU.ApuEventQueue[event].type & 3) {
      case 0:
        S.pAPU.ApuC5Freq =
            ApuDpcmCycles[(S.pAPU.ApuEventQueue[event].data & 0x0F)] << 13;
      ...
      case 1:
        ...
        break;
      case 2:
        S.pAPU.ApuC5CacheAddr =
            0xC000 + (word)(S.pAPU.ApuEventQueue[event].data << 6);
        break;
      case 3:
        S.pAPU.ApuC5CacheDmaLength =
            ((S.pAPU.ApuEventQueue[event].data << 4) + 1) << 3;
        break;
      }
    } else if (S.pAPU.ApuEventQueue[event].type == APUET_W_CTRL) {
      S.ApuCtrlNew = S.pAPU.ApuEventQueue[event].data;
...
void ApuRenderingWave5(void) {
  ...
  for (i = 0; i < ApuSamplesPerSync; i++) {
    ...
    if (S.ApuCtrlNew & 0x10) {
      ...
      wrotePCM = 1;
      wave_buffers[4][i] =
          ((S.pAPU.ApuC5Reg[1] & 0x01) + (S.pAPU.ApuC5DpcmValue * 2.5));
    } else {
      if (S.pAPU.ApuC5Looping) {
...
void NESCore_pAPU_VSync(void) {
  ...
  NESCore_Callback_OutputSample(ApuSamplesPerSync, wave_buffers[0],
                                wave_buffers[1], wave_buffers[2],
...
void NESCore_pAPU_Finish(void) { NESCore_Callback_CloseSound(); }
