...
static byte Rd6502(word wAddr) {
  ...
  case 0x4000: /* pAPU */
    ...
    case (0x4015): /* APU Control */
      ...
      if (S.pAPU.ApuC1Atl > 0)
        bScratch |= (1 << 0);
      if (S.pAPU.ApuC2Atl > 0)
        bScratch |= (1 << 1);
      if (!MC3Holdnote) {
        if (S.pAPU.ApuC3Atl > 0)
          bScratch |= (1 << 2);
      } else {
...
static void Wr6502(word wAddr, byte bData) {
  ...
  case 0x2000: /* PPU */
    ...
    case 0x2005: /* Scroll Register */
      ...
      if (S.PPU_Latch_Flag) {
        S.vAddr_Latch =
            (S.vAddr_Latch & 0x8C1F) | (upperBits << 5) | (lowerBits << 12);
      } else {
      ...
      } else if (!(wScratch & 0xF)) /* 0x3F00 or 0x3F10 */
      {
        ...
        S.PalTable[0x00] = S.PalTable[0x04] = S.PalTable[0x08] =
            S.PalTable[0x0c] = S.PalTable[0x10] = S.PalTable[0x14] =
                S.PalTable[0x18] = S.PalTable[0x1c] = NesPalette[bData];
  ...
  case 0x4000: /* pAPU and Sprite */
    ...
    case 0x00:
      ...
      S.pAPU.ApuC1Volume =
          (S.pAPU.ApuC1EnvEnabled) ? S.pAPU.ApuC1EnvVol : bData & 0xF;
      break;
    ...
    case 0x04:
      ...
      S.pAPU.ApuC2Volume =
          (S.pAPU.ApuC2EnvEnabled) ? S.pAPU.ApuC2EnvVol : bData & 0xF;
      break;
    ...
    case 0x0c:
      ...
      S.pAPU.ApuC4Volume =
          (S.pAPU.ApuC4EnvEnabled) ? S.pAPU.ApuC4EnvVol : bData & 0xF;
      break;
    ...
    case 0x14: /* 0x4014: Sprite DMA */
      ...
      switch (bData >> 5) {
      case 0x00: /* RAM */
        memcpy(S.SPRRAM, &S.RAM[((word)bData << 8) & 0x7ff], SPRRAM_SIZE);
        break;

      case 0x03: /* SRAM */
        memcpy(S.SPRRAM, &S.SRAM[((word)bData << 8) & 0x1fff], SPRRAM_SIZE);
        break;

      case 0x04: /* ROM BANK 0 */
        memcpy(S.SPRRAM, &W.ROMBANK0[((word)bData << 8) & 0x1fff], SPRRAM_SIZE);
        break;

      case 0x05: /* ROM BANK 1 */
        memcpy(S.SPRRAM, &W.ROMBANK1[((word)bData << 8) & 0x1fff], SPRRAM_SIZE);
        break;

      case 0x06: /* ROM BANK 2 */
        memcpy(S.SPRRAM, &W.ROMBANK2[((word)bData << 8) & 0x1fff], SPRRAM_SIZE);
        break;

      case 0x07: /* ROM BANK 3 */
        memcpy(S.SPRRAM, &W.ROMBANK3[((word)bData << 8) & 0x1fff], SPRRAM_SIZE);
        break;
      }
      break;
