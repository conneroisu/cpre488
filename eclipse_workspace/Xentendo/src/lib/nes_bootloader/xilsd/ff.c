...
(_FS_TINY) /                   Added long file name support. / Added multiple
code page support. /                   Added re-entrancy for multitask
operation. /                   Added auto cluster size selection to f_mkfs(). /
Added rewind option to f_readdir(). /                   Changed result code of
critical errors. /                   Renamed string functions to avoid name
collision. / Apr 14,'09 R0.07a Separated out OS dependent code on reentrant cfg.
...
/                   Changed f_readdir() to return the SFN with always upper case
on non-LFN cfg.
/
...
/                   Changed fname member in the FILINFO structure on Unicode
cfg. /                   String functions support UTF-8 encoding files on
Unicode cfg. / Aug 16,'10 R0.08a Added f_getcwd(). (_FS_RPATH = 2) / Added
sector erase feature. (_USE_ERASE) /                   Moved file lock semaphore
table from fs object to the bss. /                   Fixed a wrong directory
entry is created on non-LFN cfg when the given name contains ';'. / Fixed
f_mkfs() creates wrong FAT32 volume.
/---------------------------------------------------------------------------*/
#include "xparameters.h"
// phjones removed #ifdef XPAR_PS7_SD_0_S_AXI_BASEADDR
#ifdef XPAR_XSDPS_0_BASEADDR // phjones added to update for VIVADO xparmeter def
                             // (should update to use SDK managed functions)
#include "../../../src/lib/nes_bootloader/xilsd/diskio.h"          /* Declarations of low level disk I/O functions */
#include "../../../src/lib/nes_bootloader/xilsd/ff.h"              /* FatFs configurations and declarations */
...
typedef struct {
  ...
  WORD ctr;  /* File open counter, 0:none, 0x01..0xFF:read open count,
                0x100:write mode */
} FILESEM;
#endif
...
chk_lock(         /* Check if the file can be accessed */
  ...
  for (i = be = 0; i < _FS_SHARE; i++) {
    if (Files[i].fs) { /* Existing entry */
      if (Files[i].fs ==
              dj->fs && /* Check if the file matched with an open file */
  ...
  /* The file has been opened. Reject any open against writing file and all
   * write mode open */
  return (acc || Files[i].ctr == 0x100) ? FR_LOCKED : FR_OK;
}

static int enq_lock(/* Check if an entry is available for a new file */
...
static FRESULT dir_sdi(DIR *dj, /* Pointer to directory object */
  ...
  dj->dir = dj->fs->win +
            (idx % (SS(dj->fs) / 32)) * 32; /* Ptr to the entry in the sector */
...
static FRESULT
dir_next(/* FR_OK:Succeeded, FR_NO_FILE:End of table, FR_DENIED:EOT and could
            not stretch */
    ...
    if (dj->clust == 0) {         /* Static table */
      ...
      if (((i / (SS(dj->fs) / 32)) & (dj->fs->csize - 1)) ==
        ...
        if (clst == 0xFFFFFFFF)
          return FR_DISK_ERR;
        if (clst >=
            dj->fs->n_fatent) { /* When it reached end of dynamic table */
#if !_FS_READONLY
          BYTE c;
          if (!stretch)
...
static const BYTE LfnOfs[] = {
    1,  3,  5,  7,  9,  14, 16,
    18, 20, 22, 24, 28, 30}; /* Offset of LFN chars in the directory entry */
    ...
    if (c == 0xE5 ||
      ...
      if (a == AM_LFN) { /* An LFN entry is found */
        if (dj->lfn) {
          ...
          ord = (c == ord && sum == dir[LDIR_Chksum] && cmp_lfn(dj->lfn, dir))
                    ? ord - 1
                    : 0xFF;
        }
...
/*-----------------------------------------------------------------------*/
#if _FS_MINIMIZE <= 1
static FRESULT dir_read(DIR *dj /* Pointer to the directory object that pointing
  ...
  while (dj->sect) {
    ...
    if (c == 0xE5 || (!_FS_RPATH && c == '.') ||
      ...
      if (a == AM_LFN) { /* An LFN entry is found */
        ...
        ord = (c == ord && sum == dir[LDIR_Chksum] && pick_lfn(dj->lfn, dir))
                  ? ord - 1
                  : 0xFF;
      } else {                          /* An SFN entry is found */
...
static FRESULT create_name(DIR *dj, /* Pointer to the directory object */
  ...
  *path = &p[si];               /* Return pointer to the next segment */
  ...
  if (sfn[0] == 0xE5)
    sfn[0] =
        0x05; /* When first char collides with 0xE5, replace it with 0x05 */
...
chk_mounted(/* FR_OK(0): successful, !=0: any error occurred */
  ...
  *rfs = fs =
  ...
  /* Following code attempts to mount a volume. (analyze BPB and initialize the
   * fs object) */
  ...
  if (fs->fsize <
      (szbfat + (SS(fs) - 1)) /
          SS(fs)) { /* (FAT size must not be less than FAT sectors */
    return FR_NO_FILESYSTEM;
  }
...
FRESULT f_write(FIL *fp,          /* Pointer to the file object */
  ...
  *bw = 0; /* Initialize byte counter */
  ...
  for (; btw; /* Repeat until all data transferred */
    ...
    if ((fp->fptr % SS(fp->fs)) == 0) { /* On the sector boundary? */
      ...
      if (!csect) {                        /* On the cluster boundary? */
        if (fp->fptr == 0) {               /* On the top of the file? */
          ...
          clst = create_chain(
              fp->fs, fp->curr_clust); /* Follow or stretch cluster chain */
        }
        if (clst == 0)
      ...
      if (fp->fs->winsect == fp->dsect &&
          move_window(fp->fs, 0)) /* Write back data buffer prior to following
                                     direct transfer */
        ABORT(fp->fs, FR_DISK_ERR);
#else
      if (fp->flag & FA__DIRTY) { /* Write back data buffer prior to following
                                     direct transfer */
        if (disk_write(fp->fs->drv, fp->buf, fp->dsect, 1) != RES_OK)
          ABORT(fp->fs, FR_DISK_ERR);
        fp->flag &= ~FA__DIRTY;
      ...
      if (cc) {              /* Write maximum contiguous sectors directly */
        ...
        if (fp->fs->winsect - sect <
            cc) { /* Refill sector cache if it gets dirty by the direct write */
...
#if _FS_TINY
      if (fp->fptr >=
          fp->fsize) { /* Avoid silly buffer filling at growing edge */
    ...
    wcnt =
        SS(fp->fs) -
        (fp->fptr % SS(fp->fs)); /* Put partial sector into file I/O buffer */
    if (wcnt > btw)
      wcnt = btw;
#if _FS_TINY
    if (move_window(fp->fs, fp->dsect)) /* Move sector window */
...
#if !_FS_READONLY
    if (fp->fptr >
        fp->fsize) { /* Set change flag if the file size is extended */
      fp->fsize = fp->fptr;
      fp->flag |= FA__WRITTEN;
    }
#endif
...
FRESULT f_getfree(
    ...
    FATFS **fatfs /* Pointer to pointer to corresponding file system object to
                     return */
) {
  FRESULT res;
    ...
    if (res == FR_OK) { /* Old object is found */
      if (!djo.dir) {   /* Is root dir? */
        ...
        if (res == FR_NO_FILE) { /* Is it a valid path and no name collision? */
          /* Start critical section that any interruption or error can cause
           * cross-link */
...
FRESULT f_mkfs(BYTE drv, /* Logical drive number */
  ...
  for (i = 0; i < N_FATS; i++) {
    ...
    for (n = 1; n < n_fat; n++) { /* This loop may take a time on FAT32 volume
                                     due to many single sector write */
      if (disk_write(drv, tbl, wsect++, 1) != RES_OK)
        return FR_DISK_ERR;
    }
  }
